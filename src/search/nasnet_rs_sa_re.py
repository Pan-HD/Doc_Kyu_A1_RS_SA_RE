...
def _numeric_stats(label: str, values: Sequence[float]) -> str:
    ...
    return (
        f"{label}: min={min(numbers):.6f} max={max(numbers):.6f} "
        f"mean={mean:.6f} std={math.sqrt(variance):.6f}"
    )
...
def run_nasnet_rs_sa_re(
    *,
    ...
    # Rebound to a run.log + console sink after output files are prepared.
    diagnostic_sink: Callable[[str], None] = print_fn
    ...
    if fit_surrogate_fn is None:
        ...
        base_fit_fn = fit_records
    else:
        base_fit_fn = fit_surrogate_fn
    base_predict_fn = predict_surrogate_fn or predict_multitask

    def active_fit_fn(
        records: Sequence[PairedEvaluationRecord],
        encodings: Sequence[Any],
    ):
        result = base_fit_fn(records, encodings)
        mean_targets = [record.mean_target for record in records]
        instability_targets = [
            record.instability_target
            for record in records
            if record.instability_target is not None
        ]
        diagnostic_sink("[Surrogate training]")
        diagnostic_sink(
            f"training_size={len(records)} "
            f"paired_label_count={len(instability_targets)}"
        )
        diagnostic_sink(_numeric_stats("mu_target", mean_targets))
        diagnostic_sink(_numeric_stats("d_target", instability_targets))
        diagnostic_sink(
            f"training_loss={float(result.training_loss):.8f} "
            f"mean_mse={float(result.mean_training_mse):.8f} "
            f"instability_mse={float(result.instability_training_mse):.8f}"
        )
        return result

    def active_predict_fn(model: Any, encodings: Sequence[Any]):
        prediction = base_predict_fn(model, encodings)
        predicted_mu = tuple(
            float(value)
            for value in prediction.predicted_mean.detach().cpu().tolist()
        )
        predicted_d = tuple(
            float(value)
            for value in (
                prediction.predicted_instability.detach().cpu().tolist()
            )
        )
        if any(value < 0.0 for value in predicted_d):
            raise RuntimeError("surrogate predicted a negative instability")
        diagnostic_sink("[Surrogate prediction]")
        diagnostic_sink(_numeric_stats("predicted_mu", predicted_mu))
        diagnostic_sink(_numeric_stats("predicted_d", predicted_d))
        return prediction
    ...
    def on_progress(progress: RSSAREvolutionProgress[Any]) -> None:
        ...
        if progress.event_type == "first_evaluation":
            ...
            diagnostic_sink("[Repeat]")
            diagnostic_sink(
                f"base_eval={record.base_evaluation_index} "
                f"seed1={record.seed_1} acc1={record.accuracy_1:.6f}"
            )
            diagnostic_sink(
                f"seed2={record.seed_2} acc2={record.accuracy_2:.6f}"
            )
            diagnostic_sink(
                f"mean_target={record.mean_target:.6f} "
                f"instability={record.instability_target:.6f}"
            )
            diagnostic_sink(
                f"budget={event.budget_index}/{budget} population unchanged"
            )
        else:
        ...
        if candidate_batch is not None:
            ...
            diagnostic_sink(
                f"[RS-SA-RE budget {event.budget_index} / "
                f"first eval {candidate_batch.evaluation_index}]"
            )
            for candidate in candidate_batch.candidates:
                diagnostic_sink(
                    f"Candidate {candidate.candidate_index}: "
                    f"mu_hat={candidate.predicted_mu:.6f} "
                    f"d_hat={candidate.predicted_d:.6f} "
                    f"lambda={candidate.stability_penalty_lambda:.6f} "
                    f"score={candidate.score:.6f}"
                )
            diagnostic_sink(
                f"Selected: candidate {candidate_batch.selected_candidate_index}"
            )
