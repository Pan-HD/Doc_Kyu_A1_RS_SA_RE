...
class TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.optimizer.upper() != "SGD":
            raise ValueError("Part C provisional trainer supports only SGD")
        ...
        if self.scheduler.lower() != "cosine":
            raise ValueError(
                "Part C provisional trainer supports only cosine scheduling"
            )
        if self.gradient_clip_norm <= 0:
            raise ValueError("gradient_clip_norm must be positive")
        if self.drop_path:
            raise ValueError("DropPath is disabled for Part C benchmarks")
        if self.auxiliary_head:
            raise ValueError("auxiliary heads are disabled for Part C")
        if self.torch_compile:
            raise ValueError("torch.compile is disabled for Part C benchmarks")
...
def train_nasnet(
    ...
    """Train/evaluate one architecture for a fixed epoch budget.

    The function deliberately accepts no official-test loader. Callers should
    use ``result.final_val_accuracy`` (or ``result.fitness``) for search.
    """
