...
def _select_budget_index(
    rows: Sequence[Mapping[str, Any]],
) -> tuple[str | None, int]:
    """Select a true per-run index instead of a repeated total-budget field.

    Some evaluation logs contain both ``budget=30`` on every row (the configured
    total budget) and ``evaluation_index=1..30``.  A column is accepted as the
    horizontal budget index only when it uniquely covers every integer in either
    1..N or 0..N-1.  If no recognised column satisfies that rule, CSV/JSON row
    order is the unambiguous real-training order and is used instead.
    """

    for key in ALIASES["budget"]:
        values: list[int] = []
        for row in rows:
            raw = row.get(key)
            if raw in (None, ""):
        ...
        if sorted(values) == list(range(1, len(rows) + 1)):
            return key, 0
        if sorted(values) == list(range(0, len(rows))):
            return key, 1
    return None, 0
...
def parse_evaluations(raw_rows: Sequence[Mapping[str, Any]], source: Path) -> list[Evaluation]:
    ...
    rows = [_normalise_record(raw) for raw in raw_rows]
    budget_key, budget_offset = _select_budget_index(rows)
    for ordinal, row in enumerate(rows, start=1):
        ...
        if budget_key is None:
            ...
            raw_budget = _as_int(row[budget_key], field=budget_key)
            assert raw_budget is not None
            budget = raw_budget + budget_offset
