...
def audit_rs_sa_re_run(
    output_dir: str | Path,
    *,
    expected_mode: str,
    expected_epochs: int,
) -> SmokeAuditSummary:
    """Audit shared B=30 RS-SA-RE invariants for debug or pilot runs."""

    normalized_mode = str(expected_mode).lower()
    if normalized_mode not in {"debug", "pilot"}:
        raise ValueError("expected_mode must be debug or pilot")
    ...
    if str(experiment["mode"]).lower() != normalized_mode:
        raise ValueError(
            f"audit expected experiment.mode={normalized_mode}"
        )
    if int(training["epochs"]) != expected_epochs:
        raise ValueError(
            f"audit expected training.epochs={expected_epochs}"
        )
    ...
    if (
        ...
        raise ValueError("run config must be P=20, S=5, B=30, K=5")
    if not math.isclose(penalty_lambda, 1.0, rel_tol=0.0, abs_tol=0.0):
        raise ValueError("run audit requires provisional lambda=1.0")
...
def audit_rs_sa_re_smoke(output_dir: str | Path) -> SmokeAuditSummary:
    """Backward-compatible Part F one-epoch debug audit."""

    return audit_rs_sa_re_run(
        output_dir,
        expected_mode="debug",
        expected_epochs=1,
    )
...
def audit_rs_sa_re_pilot(output_dir: str | Path) -> SmokeAuditSummary:
    """Five-epoch Part G pilot audit using the same hard budget rules."""
