...
**Formal execution status:** Not yet authorized; runner compatibility, Part L–N
...
The uploaded `scripts/run_rs_sa_re.py` currently accepts only `debug` and
`pilot` modes and requires `B=30`. Therefore it rejects the frozen formal
RS-SA-RE configuration (`mode=formal`, `B=60`). This compatibility issue is
outside the Part H–K file-mutation boundary and is a hard blocker for formal
execution until it is corrected and covered by regression tests.
...
- resolve the formal-mode/`B=60` RS-SA-RE runner incompatibility;
- run the generator check, formal config tests, formal manifest tests, full
  regression, matched-seed/RNG preflight, and method-specific dry audits;
